pipeline {
  ...
  stages {
    ...
    success {
        ...
        sh '''
curl -sS -X POST -H "Content-Type: application/json" -d @- "$WEBHOOK" <<JSON
{ "content": "✅ ${JOB_NAME} #${BUILD_NUMBER} succeeded on ${BRANCH_NAME}\\nImage: ${IMAGE}:${GIT_SHA_SHORT}" }
JSON
        '''
        }
    }
    failure {
        ...
        sh '''
curl -sS -X POST -H "Content-Type: application/json" -d @- "$WEBHOOK" <<JSON
{ "content": "❌ ${JOB_NAME} #${BUILD_NUMBER} failed on ${BRANCH_NAME}\\nLogs: ${BUILD_URL}" }
JSON
        '''
        }
    }
